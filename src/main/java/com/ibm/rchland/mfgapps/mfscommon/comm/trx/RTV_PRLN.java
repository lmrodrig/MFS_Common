...
public class RTV_PRLN extends ServerTransaction implements Cacheable 
{
	private static Cache cache = null;
	private MFSActionable actionable;
	private String inputCellType;
	private String outputPrlnList;

	public RTV_PRLN(MFSActionable actionable) 
	{
		super();
	...
	public String getOutputPrlnList() {
		return this.outputPrlnList;
	}
	...
	protected void buildInput() {
		...
		inputBuffer.append("RTV_PRLN  ");  //$NON-NLS-1$
	...
	protected void callTransaction() {
		MFSTransaction rtv_prln = new MFSFixedTransaction(this.getInput());
		rtv_prln.setActionMessage("Retrieving List of Product Lines, Please Wait..."); //$NON-NLS-1$
		MFSComm.getInstance().execute(rtv_prln, this.actionable);
		
		this.setOutput(rtv_prln.getOutput());
		this.setReturnCode(rtv_prln.getReturnCode());
		if(this.getReturnCode() != 0) {
			this.setErrorMessage(rtv_prln.getErms());
		}		
	}
	...
	protected void parseOutput() {
		if(this.getReturnCode() == 0) {
			this.outputPrlnList = this.getOutput();
		}		
	}
